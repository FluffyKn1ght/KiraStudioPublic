...
Each column of the sequencer represents exactly 1 bar of music, and each row represent a **Track**. A track is always part of a **Channel** and a channel may have zero or more tracks. 

Please note that the names channels and tracks do not correspond to the concept of MIDI channels or tracks. This app is not concerned with MIDI beyond import or export. 
...
* **Effect Chains** only contain effects and can only use automation track to automate the value of the effect parameters.
...
Visually each row is a track and all track of a channel are the same color. Channels have a little icon next to their names.
...
The **Selected Channel** is diplayed with lighter color in the sequencer and is the channel that will be previewed when playing the pop-up piano, or pressing keys on a MIDI controller. To change the selected channel, simply click on an instrument channel.
...
Additionally, if there is an selected channel, you'll have the option to add a piano roll track to it. Having multiple piano roll track on a channel is mostly for your own organization, althrough there are some cases where it is genuinely useful (arpeggios, playing the same note twice, etc.).
...
Muting a channel is done by clicking on its icon. Visually, muted channels have their icon dimmed. A single click will mute a channel, while a double click will solo it but muting all the other channels.
...
To reorder a channel, grab the first track of the channel and you will be able to move the entire channel up or down relative to others.

To reorder a track inside a channel, grab any track that is not the first one, and you will be able to moving it relative to other tracks inside that channel. Please do note that piano roll tracks always come before automation tracks in a channel.
...
* On mobile, you can longpress somewhere and start drawing a rectangle once the pulsating circle appears. 
...
When selecting patterns this way, you will notice that the bar numbers also get a white outline around them. This is important and means that if you move these patterns left or right, the associated [custom time signatures](#custom-time-signatures) will also be moved with them. 

> TODO : A mobile/desktop GIF here would be nice.

## Copying & instancing patterns

Patterns have an associated 3-digit numbers. This number is unique for a given track and allows you to tell if a pattern is used multiple times inside a track. Re-using a pattern multiple times in a track is called **instancing**. When modifying a pattern, all of the its instances will be modified at the same time. 
